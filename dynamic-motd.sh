...
if [[ ! -f /usr/bin/figlet || ! -f /usr/bin/bc || ! -f /usr/bin/lsb_release ]]; then
	echo -e "$red\nError: Required packages are not installed.\ninstall with\napt-get update && apt-get install figlet bc lsb-release$defcolor\n"
	exit
fi
...
if [ $EUID -eq 0 ]; then
	...
	if (("$package_manager_found" == "1")); then
		...
		echo -e "$updates packages can be updated.$status_updates_security \n$updates_security updates are security updates.\n$defcolor"
	fi;
else
